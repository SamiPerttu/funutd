...
#[inline] pub fn sqrt<F: Real>(x: F) -> F { x.sqrt() }
#[inline] pub fn exp<F: Real>(x: F) -> F { x.exp() }
#[inline] pub fn log<F: Real>(x: F) -> F { x.log() }
#[inline] pub fn sin<F: Real>(x: F) -> F { x.sin() }
#[inline] pub fn cos<F: Real>(x: F) -> F { x.cos() }
#[inline] pub fn tan<F: Real>(x: F) -> F { x.tan() }

impl<F: num_traits::real::Real> Real for F {
    #[inline] fn sqrt(self) -> F { self.sqrt() }    
    #[inline] fn exp(self) -> F { self.exp() }
    #[inline] fn log(self) -> F { self.ln() }
    #[inline] fn sin(self) -> F { self.sin() }
    #[inline] fn cos(self) -> F { self.cos() }
    #[inline] fn tan(self) -> F { self.tan() }
}
...
impl<T : Copy> At<T> for Vec<T> {
    #[inline] fn at<I: AsPrimitive<usize> >(&self, i: I) -> T { self[i.as_()] }
}
...
/// Minimum of x and y.
#[inline] pub fn min<T: Min>(x: T, y: T) -> T { x.min(y) }
...
/// Maximum of x and y.
#[inline] pub fn max<T: Max>(x: T, y: T) -> T { x.max(y) }
...
impl<T: num_traits::Zero> Zero for T { #[inline] fn zero() -> Self { T::zero() } }
impl<T: num_traits::One> One for T { #[inline] fn one() -> Self { T::one() } }

/// Clamps x between min and max.
#[inline] pub fn clamp<T: Max + Min>(min: T, max: T, x: T) -> T { x.max(min).min(max) }

/// Clamps x between 0 and 1.
#[inline] pub fn clamp01<T: Min + Max + Zero + One>(x: T) -> T { x.max(T::zero()).min(T::one()) }
...
#[inline] pub fn lerp01<U: Lerp<T>, T: Min + Max + Zero + One>(a: U, b: U, t: T) -> U { lerp(a, b, clamp01(t)) }

#[inline] pub fn delerp<T: Copy + Sub<Output = T> + Div<Output = T>>(a: T, b: T, x: T) -> T { (x - a) / (b - a) }

#[inline] pub fn xerp<U: Lerp<T> + Real, T>(a: U, b: U, t: T) -> U { exp(lerp(log(a), log(b), t)) }

#[inline] pub fn xerp01<U: Lerp<T> + Real, T: Min + Max + Zero + One>(a: U, b: U, t: T) -> U { exp(lerp(log(a), log(b), clamp01(t))) }

#[inline] pub fn dexerp<T: Copy + Div<Output = T> + Real>(a: T, b: T, x: T) -> T { log(x / a) / log(b / a) }
...
#[inline] pub fn gray(x: u32) -> u32 {
    x ^ (x >> 1)
}

/// Decodes from binary reflected Gray code.
#[inline] pub fn degray(x: u32) -> u32 {
    let x = x ^ (x >> 16);
    let x = x ^ (x >> 8);
    let x = x ^ (x >> 4);
    let x = x ^ (x >> 2);
...
#[inline] pub fn discretize<F: FloatCore>(x: F, step: F) -> F { (x / step).round() * step }
...
#[inline] pub fn spline<F: FloatCore + 'static>(y0: F, y1: F, y2: F, y3: F, x: F) -> F where i8: AsPrimitive<F> {
...
/// Logistic sigmoid.
#[inline] pub fn logistic<F: Real + FloatCore>(x: F) -> F {
    F::one() / (F::one() + exp(-x))
}

/// Derivative of the logistic sigmoid.
#[inline] pub fn logistic_d<F: Real + FloatCore>(x: F) -> F {
    let y = logistic(x);
    y * (F::one() - y)
}

/// Softsign function.
#[inline] pub fn softsign<F: FloatCore>(x: F) -> F {
    x / (F::one() + x.abs())
}

/// Derivative of the softsign function.
#[inline] pub fn softsign_d<F: FloatCore>(x: F) -> F {
